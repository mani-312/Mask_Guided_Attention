...
class AttentionStore(AttentionBase):
    @staticmethod
    def get_self_empty_store():
        return {"down_self": [],  "mid_self": [],  "up_self": []}
    
    @staticmethod
    def get_cross_empty_store():
    ...
    def __init__(self, res=[32], min_step=0, max_step=1000):
        ...
        self.self_attns_step = self.get_self_empty_store()  # store the attns in each step
        self.cross_attns_step = self.get_cross_empty_store()
    ...
    def after_step(self):
        if self.cur_step > self.min_step and self.cur_step < self.max_step:
            ...
            if len(self.self_attns) == 0:
                ...
                for key in self.self_attns:
                    for i in range(len(self.self_attns[key])):
                        self.self_attns[key][i] += self.self_attns_step[key][i]
                
                for key in self.cross_attns:
                    for i in range(len(self.cross_attns[key])):
                        self.cross_attns[key][i] += self.cross_attns_step[key][i]
        # self.self_attns_step.clear()
        # self.cross_attns_step.clear()

        self.self_attns_step = self.get_self_empty_store()
        self.cross_attns_step = self.get_cross_empty_store() 
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if attn.shape[1] <= 64 ** 2:  # avoid OOM
            if is_cross:
                self.cross_attns_step[key].append(attn)
            else:
                self.self_attns_step[key].append(attn)
        return super().forward(q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs)
    
    def get_average_attention(self):
        average_attention = {key: [item / self.cur_step for item in self.cross_attns[key]] for key in self.cross_attns}
