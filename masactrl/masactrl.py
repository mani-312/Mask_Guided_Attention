...
class MutualSelfAttentionControlMask_ComposeSyntheisized(MutualSelfAttentionControl):
    def __init__(self,  start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50,  mask_s=None, mask_t=None, mask_new_object = None, mask_save_dir=None, model_type="SD"):
        """
        ...
        self.mask_new = mask_new_object # Mask for new subject
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        # new
        out_u_target_selfa = self.attn_batch(qu[-num_heads:], ku[-num_heads:], vu[-num_heads:], sim[-num_heads:], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_c_target_selfa = self.attn_batch(qc[-num_heads:], kc[-num_heads:], vc[-num_heads:], sim[-num_heads:], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        ...
        if self.mask_s is not None and self.mask_t is not None:
            ...
            if self.mask_new is not None:
                mask = F.interpolate(self.mask_new.unsqueeze(0).unsqueeze(0), (H, W))
                mask = mask.reshape(-1, 1)  # (hw, 1)
                out_u_target = out_u_target_selfa * mask + out_u_target * (1 - mask)
                out_c_target = out_c_target_selfa * mask + out_c_target * (1 - mask)

        out = torch.cat([out_u_source, out_u_target, out_c_source, out_c_target], dim=0)
        return out

# order of data source, target, reference
class MutualSelfAttentionControlMask_ComposeReference(MutualSelfAttentionControl):                        #corgi      #corgi         #human             #human 
    def __init__(self,  start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50,  mask_s=None, mask_t_s=None, mask_t_ref = None, mask_ref = None,mask_save_dir=None, model_type="SD"):
        """
        Maske-guided MasaCtrl to alleviate the problem of fore- and background confusion
        Args:
            start_step: the step to start mutual self-attention control
        ...
        super().__init__(start_step, start_layer, layer_idx, step_idx, total_steps, model_type)
        self.mask_s = mask_s  # source mask with shape (h, w)
        self.mask_t_s = mask_t_s  # target mask with same shape as source mask
        self.mask_t_ref = mask_t_ref # Mask for new subject
    ...
    def attn_batch(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, mask = None, **kwargs):
        B = q.shape[0] // num_heads
        H = W = int(np.sqrt(q.shape[1]))
        q = rearrange(q, "(b h) n d -> h (b n) d", h=num_heads)
        k = rearrange(k, "(b h) n d -> h (b n) d", h=num_heads)
        ...
        sim = torch.einsum("h i d, h j d -> h i j", q, k) * kwargs.get("scale")
        if kwargs.get("is_mask_attn") and mask is not None:
            # print("masked attention")
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        out_u_ref = self.attn_batch(qu[-num_heads:], ku[-num_heads:], vu[-num_heads:], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_c_ref = self.attn_batch(qc[-num_heads:], kc[-num_heads:], vc[-num_heads:], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        # new
        # out_u_target_selfa = self.attn_batch(qu[-num_heads:], ku[-num_heads:], vu[-num_heads:], sim[-num_heads:], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        # out_c_target_selfa = self.attn_batch(qc[-num_heads:], kc[-num_heads:], vc[-num_heads:], sim[-num_heads:], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        out_u_target_src = self.attn_batch(qu[num_heads:2*num_heads], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, mask = self.mask_s, is_mask_attn=True, **kwargs)
        out_c_target_src = self.attn_batch(qc[num_heads:2*num_heads], kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, mask = self.mask_s, is_mask_attn=True, **kwargs)

        out_u_target_ref = self.attn_batch(qu[num_heads:2*num_heads], ku[-num_heads:], vu[-num_heads:], sim[-num_heads:], attnu, is_cross, place_in_unet, num_heads, mask = self.mask_ref, is_mask_attn=True, **kwargs)
        out_c_target_ref = self.attn_batch(qc[num_heads:2*num_heads], kc[-num_heads:], vc[-num_heads:], sim[-num_heads:], attnc, is_cross, place_in_unet, num_heads, mask = self.mask_ref, is_mask_attn=True, **kwargs)

        if self.mask_s is not None and self.mask_t_s is not None:
            out_u_target_src_fg, out_u_target_src_bg = out_u_target_src.chunk(2, 0)
            out_c_target_src_fg, out_c_target_src_bg = out_c_target_src.chunk(2, 0)

            mask = F.interpolate(self.mask_t_s.unsqueeze(0).unsqueeze(0), (H, W))
            mask = mask.reshape(-1, 1)  # (hw, 1)
            out_u_target_src = out_u_target_src_fg * mask + out_u_target_src_bg * (1 - mask)
            out_c_target_src = out_c_target_src_fg * mask + out_c_target_src_bg * (1 - mask)

            # new
            if self.mask_t_ref is not None:
                out_u_target_ref_fg, out_u_target_bg = out_u_target_ref.chunk(2, 0)
                out_c_target_ref_fg, out_c_target_bg = out_c_target_ref.chunk(2, 0)

                mask = F.interpolate(self.mask_t_ref.unsqueeze(0).unsqueeze(0), (H, W))
                mask = mask.reshape(-1, 1)  # (hw, 1)
                out_u_target = out_u_target_ref_fg * mask + out_u_target_src * (1 - mask)
                out_c_target = out_c_target_ref_fg * mask + out_c_target_src * (1 - mask)

        out = torch.cat([out_u_source, out_u_target, out_u_ref, out_c_source, out_c_target, out_c_ref], dim=0)
        return out

# three sampling
# Source gamrnet, target without edit, target with edit
class MutualSelfAttentionControlMask_AT_StructureBackground(MutualSelfAttentionControl):
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        # 0:num_heads -> source
        # num_heads:2*num_heads -> target no edit
        # -num_heads: -> target with edit
        """
        Attention forward function
        """
        if is_cross or self.cur_step not in self.step_idx or self.cur_att_layer // 2 not in self.layer_idx:
        ...
        out_u_target_ne = self.attn_batch(qu[num_heads:2*num_heads], ku[num_heads:2*num_heads], vu[num_heads:2*num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_c_target_ne = self.attn_batch(qc[num_heads:2*num_heads], kc[num_heads:2*num_heads], vc[num_heads:2*num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        # new
        # out_u_target_selfa = self.attn_batch(qu[-num_heads:], ku[-num_heads:], vu[-num_heads:], sim[-num_heads:], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        # out_c_target_selfa = self.attn_batch(qc[-num_heads:], kc[-num_heads:], vc[-num_heads:], sim[-num_heads:], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        out_u_target_src = self.attn_batch(qu[-num_heads:], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, mask = self.mask_s, is_mask_attn=True, **kwargs)
        out_c_target_src = self.attn_batch(qc[-num_heads:], kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, mask = self.mask_s, is_mask_attn=True, **kwargs)

        out_u_target_target_ne = self.attn_batch(qu[-num_heads:], ku[num_heads:2*num_heads], vu[num_heads:2*num_heads], sim[-num_heads:], attnu, is_cross, place_in_unet, num_heads, mask = self.mask_t, is_mask_attn=True, **kwargs)
        out_c_target_target_ne = self.attn_batch(qc[-num_heads:], kc[num_heads:2*num_heads], vc[num_heads:2*num_heads], sim[-num_heads:], attnc, is_cross, place_in_unet, num_heads, mask = self.mask_t, is_mask_attn=True, **kwargs)

        if self.mask_s is not None and self.mask_t is not None:
            out_u_target_src_fg, _ = out_u_target_src.chunk(2, 0)
            out_c_target_src_fg, _ = out_c_target_src.chunk(2, 0)

            _, out_u_target_target_ne_bg = out_u_target_target_ne.chunk(2, 0)
            _, out_c_target_target_ne_bg = out_c_target_target_ne.chunk(2, 0)

            mask = F.interpolate(self.mask_t.unsqueeze(0).unsqueeze(0), (H, W))
            mask = mask.reshape(-1, 1)  # (hw, 1)
            out_u_target = out_u_target_src_fg * mask + out_u_target_target_ne_bg * (1 - mask)
            out_c_target = out_c_target_src_fg * mask + out_c_target_target_ne_bg * (1 - mask)

        out = torch.cat([out_u_source, out_u_target_ne, out_u_target, out_c_source, out_c_target_ne, out_c_target], dim=0)
        return out
...
# three sampling
# Source gamrnet, target without edit, target with edit
class MutualSelfAttentionControlMask_AT_AppearanceBackground(MutualSelfAttentionControl):
    def __init__(self,  start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50,  mask_s=None, mask_t=None, mask_save_dir=None, model_type="SD"):
        """
        Maske-guided MasaCtrl to alleviate the problem of fore- and background confusion
        Args:
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if self.mask_s is not None and self.mask_t is not None:
            ...
            mask = F.interpolate(self.mask_t.unsqueeze(0).unsqueeze(0), (H, W))
            mask = mask.reshape(-1, 1)  # (hw, 1)
            out_u_target = out_u_target_src_fg * mask + out_u_target_src_bg * (1 - mask)
            out_c_target = out_c_target_src_fg * mask + out_c_target_src_bg * (1 - mask)
